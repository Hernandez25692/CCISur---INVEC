        ...
        <div class="max-w-7xl mx-auto">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
                <div>
                    ...
                    <p class="text-sm text-gray-600 mt-1">Listado completo de asignaciones de equipos y mobiliario</p>
                </div>
                ...
                <form method="GET" action="{{ route('asignaciones.index') }}">
                    <div class="flex flex-col md:flex-row gap-4">
                        ...
                        <div class="flex items-center gap-3">
                            <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-md text-sm font-medium transition duration-150 ease-in-out whitespace-nowrap">
                                Buscar
                            </button>
                            <a href="{{ route('asignaciones.index') }}" class="text-sm text-gray-600 hover:text-gray-900 hover:underline transition">
                                Limpiar
                            </a>
                        </div>
                    </div>
            ...
            <div class="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-100">
                ...
                <div class="text-center py-12">
                    ...
                    <div class="mt-6">
                        <a href="{{ route('asignaciones.create') }}" class="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            @php
                                $referencia = $item->tipo === 'mobiliario' 
                            ...
                            <tr class="hover:bg-gray-50 transition duration-150 ease-in-out">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-gray-900">{{ $item->empleado->nombre_completo ?? 'N/A' }}</div>
                                    <div class="text-sm text-gray-500">{{ $item->empleado->codigo ?? '' }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 capitalize">{{ $item->tipo }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ $referencia->nombre ?? 'N/A' }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 font-mono">{{ $referencia->etiqueta ?? '---' }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $item->area }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $item->fecha_entrega }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex items-center justify-end space-x-3">
                                        ...
                                        <form action="{{ route('asignaciones.destroy', $item->id) }}" method="POST" class="inline">
                                            @csrf @method('DELETE')
                                            <button onclick="return confirm('¿Está seguro de eliminar esta asignación?')" 
