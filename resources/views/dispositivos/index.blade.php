        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between h-16 items-center">
                <div class="flex-shrink-0 flex items-center">
                    <h1 class="text-xl font-bold text-gray-800">Gestión de Dispositivos</h1>
                </div>
    ...
    <div class="pt-20 pb-8 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        ...
        <div class="mb-10 relative flex flex-col md:flex-row items-center justify-between gap-6">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-4xl font-black text-gray-900 tracking-tight drop-shadow-xl">Inventario de Dispositivos</h1>
                    <p class="text-lg text-gray-500 mt-1 italic font-medium">Control total de equipos tecnológicos</p>
                </div>
            </div>
            <a href="{{ route('dispositivos.create') }}"
        ...
        <form method="GET" action="{{ route('dispositivos.index') }}" class="mb-8 bg-gradient-to-br from-white via-blue-50 to-blue-100 p-8 rounded-3xl shadow-2xl border border-blue-200">
            <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-8">
                ...
                <div>
                    <label class="block text-sm font-bold text-indigo-700 mb-2">Marca</label>
                    <input type="text" name="marca" value="{{ request('marca') }}"
                        class="border-indigo-300 focus:ring-indigo-500 focus:border-indigo-500 rounded-xl shadow focus:shadow-xl block w-full px-4 py-2 sm:text-sm bg-white transition-all duration-200"
                        placeholder="Ej. Dell, HP">
                </div>
                <div>
                    <label class="block text-sm font-bold text-indigo-700 mb-2">Modelo</label>
                    <input type="text" name="modelo" value="{{ request('modelo') }}"
                        class="border-indigo-300 focus:ring-indigo-500 focus:border-indigo-500 rounded-xl shadow focus:shadow-xl block w-full px-4 py-2 sm:text-sm bg-white transition-all duration-200"
                        placeholder="Ej. G5, Inspiron">
                </div>
                <div>
                        ...
                        @foreach ($dispositivos as $dispositivo)
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-gray-800">{{ $dispositivo->nombre }}</div>
                                    <div class="text-xs text-gray-500">{{ $dispositivo->tipo }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-gray-800">{{ $dispositivo->marca }}</div>
                                    <div class="text-xs text-gray-500">{{ $dispositivo->modelo }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @php
                                        $statusClasses = [
                                        ...
                                        $statusClass = $statusClasses[$dispositivo->estado] ?? $defaultClass;
                                    @endphp
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex justify-end space-x-2">
                                        ...
                                        @if ($dispositivo->disponibilidad === 'Sin Asignar')
                                            <a href="{{ route('asignaciones.create', ['tipo' => 'dispositivo', 'id' => $dispositivo->id]) }}"
